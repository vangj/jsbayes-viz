(function (window) {
  ...
  const OUT_LEFT = 1,
    ...
    OUT_RIGHT = 4,
    OUT_BOTTOM = 8;
  const NODE_WIDTH = 220, // 220 is the result of css calc(205px + 1rem)
    XPAD = 8,
    YPAD = 8;
  ...
  function drawNodeBars(graph) {
    ...
    for (let i = 0; i < graph.nodes.length; i++) {
      ...
      for (let j = 0; j < node.values.length; j++) {
        ...
        d3.select(`rect[data-node="${node.id}"][data-value="${value}"]`).attr({
          width: prob,
        });

        d3.select(`text[data-node="${node.id}"][data-pvalue="${value}"]`).text(
          formatPct(node.probs[j]),
        );
      }
    }
  }
  ...
  function drawNodes(options) {
    ...
    const drag = d3.behavior
      ...
      .on('drag', function (e) {
        ...
        d3.selectAll(`line[data-parent=${e.id}]`).each(function (d) {
        ...
        d3.selectAll(`line[data-child=${e.id}]`).each(function (d) {
          const points = graph.edge(d.parent, d.child);
          d3.select(this).attr({
            x1: points.x1,
            y1: points.y1,
  ...
  if (
    ...
    if (typeof window.jsbayesviz === 'undefined') {
      dagre = window.dagre;
      d3 = window.d3;
